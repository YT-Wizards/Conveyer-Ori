 ...
 *
 * Override via CONVEYER_ORI_DATA_DIR environment variable.
 */
export const DATA_DIR =
  process.env.CONVEYER_ORI_DATA_DIR ??
  path.join(os.homedir(), ".conveyer-ori");
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

const db = new Database(path.join(DATA_DIR, "ori.db"));
