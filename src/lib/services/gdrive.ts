 ...
 * and `Conveyer/Runs` in the user's Drive root and persists the IDs.
 */
export async function ensureTopLevelFolders(): Promise<{
  ...
  let runsId = getSetting("GDRIVE_RUNS_FOLDER_ID");
  ...
  if (!finalId || !runsId) {
    ...
    if (!runsId) {
      runsId = await findOrCreateFolder("Runs", rootFolder);
      setSetting("GDRIVE_RUNS_FOLDER_ID", runsId);
    }
  }
  return { finalVideosId: finalId, runsFolderId: runsId };
}
